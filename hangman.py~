...
import string # Test input is a string
...
#    word = random.choice(WORDS)
    ...
    while sizeHangman < MAX:
        ...
        # Check that the input is a single character string
	while True:
	    guess = raw_input('Guess a letter: ').lower()

            if len(guess) == 1:
                if guess in string.letters:
                    break
...
def main():
    ...
    while True:
	    ...
	    if score != 0: print('{user_name}, you found the {score} letter word in {duration} seconds'.format(**locals()))
            print('\n')
